...
##### Simon Dellicour & Marius Gilbert (26-10-2017)
***
The continuous phylogeographic method developed by Lemey *et al*. (2010) and implemented in the software BEAST (Drummond *et al*. 2012) can be used to reconstruct virus dispersion history in a continuous space. This method relies on the geographic coordinates associated with the sampling locations of virus sequences. Yet, frequently, only the more or less broad administrative area of origin of each viral sequence is known (e.g. a broad administrative area "admin-1" or only the country of origin). These low precision of sampling locations can represent a notable limitation in the field of viral phylogeography. Indeed, when precise coordinates are unavailable for sampling locations, a common practice is either to discard the sequence form that kind of analyses or to take the centroid coordinates of the administrative area within which the sequence has been sampled. However, the later option can be inappropriate. Indeed, in many cases, taking the centroid coordinates can be irrelevant, e.g., regarding the very low or even unlikely probability that the sequence has been sampled around these coordinates. The idea of this tutorial is to describe a method allowing to define smaller polygons within the overall administrative area, and associated all these smaller polygons with a distinct sampling probability. This generated collection of "sub-polygons" will then be used by BEAST to define the prior distribution of sampling coordinates associated with each sampled sequence. The idea behind this approach is to allow an appropriate use of virus sequences with low sampling precision in continuous phylogeographic reconstructions. The following approach thus proposes to include these sequences in the analysis but while using external information such as epidemiological records or host densities to specify a sampling probability assigned to some sub-polygons (admin-2 polygons) contained in the overall administrative area of origin (admin-1 polygon). We here illustrate how to use R to prepare such sub-polygons for a continuous phylogeographic analysis of the H5N1 clade-1 spread through the Mekong region. The data set is made of 214 RNA sequences (HA gene). While the admin-2 of origin is known for 109 of them, only the admin-1 polygon of origin if known for the remaining 105 sequences (see Figure 2 for the related sampling map associated clade 1).
...
library(raster)
```
**Step 2: loading admin-1 and admin-2 borders**
```
...
```
The "precisions" values indicate the level precision of the sampling locations, i.e. if we know the admin-2 or only the admin-1 administrative area of origin.
...
```
By multiplying raster cell density value by their corresponding cell area, we can then transform density measures by estimates of absolute numbers of host individuals:
```
...
hosts[!is.na(hosts[])] = log10(hosts[!is.na(hosts[])]+1)
```
Note that we here sum chicken and duck numbers to obtain an overall "host" number for each raster cell. Finally, we also log-transformed resulting numbers in order to avoid providing an excessive importance to a few raster cells associated with high values.
...
for (i in 1:length(admin_2@polygons)) {
	...
	hosts_counts = c(hosts_counts, hCounts)	
}
```
...
For sequence already assigned to an admin-2 polygon, the following script will simply generate a KML file specifying a single admin-2 polygon with an associated sampling probability equal to 1. As for sequences assigned to an admin-1 polygon, the script will then generate a KML file specifying related admin-2 polygons each associated with a specific sampling probability (the sampling probabilities of the different admin-2 polygons contained in the admin-1 polygon summing to 1). If possible, the sampling probability assigned to each admin-2 polygon will be estimated according to the H5N1 records. In that case, the sampling probability assigned to an admin-2 polygon is simply the number of H5N1 records associated to this polygon divided by the total number of H5N1 records in the admin-1 polygon. It is worth noting that to estimate these sampling probabilities we only consider H5N1 records of the sampling year of the considered sequence. When no annual H5N1 record is available for at least one of the admin-2 polygon, we then use host numbers to estimate the sampling probability assigned to these polygons. In that case, we follow the same logic as for H5N1 records but this time by estimating a sampling probability as the proportion of hosts in the admin-2 polygon compared to the total number hosts in the admin-1 polygon.
```
admins = list()
admins[[1]] = admin_1
admins[[2]] = admin_2
...
This last step is optional as XML files can also be manually edited to add text parts linking the analysis to different KML files generated above. As detailed in the resulting example file "H5N1_clade1.xml" available with this tutorial, four new parts have to be added in the XML file to refer to these sampling polygons. For each sampled sequence: (i) a new "leafTraitParameter", (ii) a "flatGeoSpatialPrior" referencing the external KML file, and (iii) an "uniformGeoSpatialOperator" have to be defined. In addition, "flatGeoSpatialPriors" have also to be listed in the operators section (see the content of the KML file for further details).
```
